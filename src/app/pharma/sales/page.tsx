      ...
      >
        {fefoBatches.map(b => (
          <option key={b.id} value={b.id}>
            {b.batchNo} · Exp: {b.expDate} (Stock: {b.stock} {item.unit}) - {rs(b.salePrice)}
          </option>
        ))}
      </select>
      ...
      {selected && (
        <div className="mt-1 flex items-center justify-between text-[10px] text-slate-500">
          <span className="flex items-center gap-1.5">
            <span>Exp: <strong className="font-medium text-slate-700">{selected.expDate}</strong></span>
...
export default function SalesPage() {
  ...
  function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (!form.vchNo.trim() || validItems.length === 0) {
      return;
    }
      ...
      <div className="rounded-xl bg-[#044d73] px-6 py-5 text-white shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/70 mt-0.5">
            Sales invoices, cash & credit billing with batch inventory tracking
          </p>
        </div>
        <button
          onClick={openAdd}
          ...
          <div className="bg-white border border-slate-200 w-full max-w-5xl max-h-[94vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">
                    Select customer, items, and batch details to generate sale voucher
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSave} className="flex min-h-0 flex-1 flex-col">
              <div className="flex-1 space-y-6 overflow-y-auto p-7">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
                    ...
                    <Field label="Payment Type">
                      <select
                        ...
                        onChange={e =>
                          setForm(p => ({ ...p, paymentType: e.target.value as PaymentType }))
                        }
                        className={inputCls}
                      >
                ...
                {/* Line Items & Batch Selection */}
                <Section
                  title="Items & Batch Allocation"
