...
/* ------------------------------------------------------------------ */
/* Types — matches the real API response shape                         */
/* ------------------------------------------------------------------ */
...
export default function CustomersPage() {
  ...
  const [search, setSearch] = useState(""); // debounced value actually sent to the backend
  // statusFilter is client-side only — listCustomersQuerySchema has no
  // status param, so this filters only the currently-loaded page.
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      // Non-critical — leave previously loaded stats displayed.
    }
  }, []);
  ...
  // Debounce search before it hits the network.
  useEffect(() => {
  ...
  // Real server-side pagination + search — refetches on page or search change.
  ...
  // Active/Inactive counts reflect the current page only — a true global
  // count would need a separate backend aggregate query.
  const activeCount = customers.filter((c) => c.status).length;
  const inactiveCount = customers.filter((c) => !c.status).length;
  ...
  function applyStatsDelta(prev: Customer | null, next: Customer | null) {
    setStats((s) => {
  ...
  async function handleSaveCustomer() {
    ...
    try {
      if (editingCustomer) {
        ...
        applyStatsDelta(editingCustomer, updated);
        setCustomers((prev) =>
        ...
        if (
          currentPage === 1 &&
          (statusFilter === "All" ||
            (statusFilter === "Active") === created.status)
        ) {
          setCustomers((prev) => [created, ...prev].slice(0, PAGE_LIMIT));
        }
        setPagination((p) => ({
          ...p,
          total: p.total + 1,
  ...
  async function toggleStatus(customer: Customer) {
    ...
    try {
      ...
      applyStatsDelta(updated, customer);
    }
  }
                    ...
                    filtered.map((customer) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          ...
                          >
                            <span
                              className={`mr-1.5 h-1.5 w-1.5 rounded-full ${customer.status ? "bg-emerald-600" : "bg-slate-400"}`}
                            />
