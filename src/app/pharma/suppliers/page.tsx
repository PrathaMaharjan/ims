...
export default function SuppliersPage() {
  ...
  async function loadSuppliers() {
    ...
    try {
      const res = await api.get("/api/supplier", {
        params: { page: currentPage, limit: PAGE_LIMIT, search: search || undefined },
      });
  ...
  const filtered = suppliers.filter((s) => {
    const matchesStatus =
      statusFilter === "All" || (statusFilter === "Active" ? s.status : !s.status);
  ...
  async function handleSaveSupplier() {
    ...
    try {
      if (editingSupplier) {
        await api.patch(`/api/supplier/${editingSupplier.id}`, payload);
      } else {
        await api.post("/api/supplier", payload);
      }
      resetForm();
      setIsModalOpen(false);
      await loadSuppliers(); // refetch the current page/search state from the server
    } catch {
  ...
  async function confirmDeleteSupplier() {
    ...
    try {
      ...
      await loadSuppliers(); // refetch — deleting the last item on a page can change totalPages
    } catch {
      setLoadError("Failed to delete supplier — it may be referenced by existing purchases.");
      setDeleteTarget(null);
    } finally {
      setDeleting(false);
    }
  ...
  async function toggleStatus(supplier: Supplier) {
    ...
    // Optimistic update on the current page's list only.
          ...
          <>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.length === 0 ? (
                    ...
                    <tr key={supplier.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-4 text-slate-500 font-mono text-xs">{supplier.panVatNumber || "—"}</td>
                      <td className="py-4 px-4">
                        <button
                          type="button"
                          onClick={() => toggleStatus(supplier)}
                          title="Click to toggle status"
                          className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium cursor-pointer transition-all hover:scale-105 active:scale-95 select-none ${supplier.status
                            ? "bg-emerald-50 text-emerald-700 border border-emerald-200/50"
                            : "bg-slate-100 text-slate-500 border border-slate-200"
                            }`}
                        >
                          <span className={`mr-1.5 h-1.5 w-1.5 rounded-full ${supplier.status ? "bg-emerald-600" : "bg-slate-400"}`} />
                ...
                <div key={supplier.id} className="p-4 flex flex-col gap-4 bg-white">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => toggleStatus(supplier)}
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium cursor-pointer transition-all select-none ${supplier.status
                        ? "bg-emerald-50 text-emerald-700 border border-emerald-200/50"
                        : "bg-slate-100 text-slate-500 border border-slate-200"
                        }`}
                    >
            ...
            <div className="flex flex-col gap-4 items-center justify-between border-t border-slate-100 bg-white px-6 py-4 sm:flex-row">
              <div className="text-sm text-slate-500 hidden sm:block">
                {pagination && `Showing ${suppliers.length} of ${pagination.total} suppliers`}
              </div>
